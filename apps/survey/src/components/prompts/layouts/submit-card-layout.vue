    ...
    <v-card :tile="isMobile">
      ...
      <div v-if="isMobile" id="actions" class="bottom-navigation">
        <div v-if="showSummary" class="bottom-navigation__summary">
          <review-meal-list-mobile v-bind="{ meals }" @action="action"></review-meal-list-mobile>
        </div>
...
import { ReviewMealListMobile } from '@intake24/survey/components/layouts/meal-list';

import layoutMixin from './layout-mixin';

export default defineComponent({
  name: 'SubmitCardLayout',

  components: { ReviewMealListMobile },
