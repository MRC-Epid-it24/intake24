...
import { filterForAddons } from '@intake24/survey/dynamic-recall/filters';
...
const addons = computed(() => filterForAddons(survey, props.prompt, meal.value));
...
const meals = computed(() => {
  ...
  if (mealEntry) {
    ...
    return [{
      ...mealEntry,
      foods: mealEntry.foods.filter(food => addons.value[mealEntry.id][food.id].length),
    }].filter(meal => meal.foods.length);
  }
  ...
  return survey.meals.map(meal => ({
    ...meal,
    foods: meal.foods.filter(food => addons.value[meal.id][food.id].length),
  })).filter(meal => meal.foods.length);
});
