<template>
  <div>
    <component :is="dialog ? `food-browser-dialog` : `v-card`" v-model="dialog" :flat="!dialog">
      <div class="pb-2" :class="{ 'px-4 pt-4': dialog }">
        <v-text-field
          ...
          :placeholder="promptI18n.search" prepend-inner-icon="$search" :rounded="dialog ? 'pill' : undefined" variant="outlined"
          @focus="openInDialog"
        />
      </div>
      <div v-if="recipeBuilderToggle" :class="$vuetify.display.mobile ? 'pa-4' : 'py-2'">
        <v-btn
          v-for="recipeBuilderFood in recipeBuilderFoods" :key="recipeBuilderFood.code" :block="$vuetify.display.mobile"
          class="mb-2 me-2" color="primary" :disabled="!recipeBuilderToggle" size="large" variant="outlined" @click.stop="recipeBuilder(recipeBuilderFood.code)"
        >
      ...
      <v-tabs-window v-show="type === 'foodSearch' || dialog || !showInDialog" v-model="tab">
        <v-tabs-window-item key="browse">
          <v-card v-if="requestFailed" flat>
            ...
            <v-card-actions>
              <v-btn size="large" @click="browseCategory(retryCode, false)">
                {{ $t('common.errors.retry') }}
              </v-btn>
            </v-card-actions>
          </v-card>
          <v-btn v-if="navigationHistory.length" size="large" variant="text" @click="navigateBack">
      ...
      >
        <v-btn
          v-if="type === 'foodSearch' && tab === 1" color="primary" :disabled="missingDialog" size="large" :title="promptI18n.browse"
          variant="outlined" @click.stop="browseRootCategory"
        >
          {{ promptI18n.browse }}
        </v-btn>
        <v-btn
          class="btn-truncate" color="primary" :disabled="missingDialog" size="large" :title="promptI18n['missing.label']"
          variant="outlined" @click.stop="openMissingDialog"
        >
          {{ promptI18n['missing.label'] }}
        </v-btn>
        <v-btn
          v-if="type === 'recipeBuilder' && !requiredToFill" class="btn-truncate" color="primary"
          :disabled="missingDialog" size="large" :title="promptI18n['missing.irrelevantIngredient']" variant="outlined"
    ...
    <missing-food-panel
      v-model="missingDialog" :class="{ 'mt-4': $vuetify.display.mobile }" :i18n="promptI18n"
...
defineOptions({ name: 'FoodBrowser', components: { FoodBrowserDialog } });
...
async function closeMissingDialog() {
  ...
  setTimeout(async () => {
    ...
    await goTo(searchRef.value.$el as HTMLElement, {
      duration: 500,
    });
