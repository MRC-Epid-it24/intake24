...
const distance = (p0: [number, number], p1: [number, number]): number => {
  const x = p1[0] - p0[0];
  const y = p1[1] - p0[1];
...
const closestVertexIndex = (vertices: [number, number][], p: [number, number]): number => {
  let closestIndex = -1;
...
const closestSegmentIndex = (vertices: [number, number][], p: [number, number]): number => {
