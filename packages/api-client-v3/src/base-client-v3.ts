...
import type { RefreshResponse } from '@intake24/common/types/http';

import type { SignInRequestV3, SignInResponseV3 } from './types';
...
export class BaseClientV3 {
  ...
  private async refresh(): Promise<RefreshResponse> {
    ...
    const response = await this.requestLimit(() =>
      this.rawClient.post<RefreshResponse>('/refresh', undefined, {
  ...
  private async signIn(): Promise<void> {
    ...
    const response = await this.requestLimit(() =>
      this.rawClient.post<SignInResponseV3>('/signin', requestData, {
