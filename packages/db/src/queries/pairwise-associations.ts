...
SELECT :localeCode, antecedent_food_code, consequent_food_code, occurrences FROM pairwise_associations_co_occurrences WHERE locale_id = :sourceLocaleCode;`;

  const transactionsCount = `INSERT INTO pairwise_associations_transactions_count (locale_id, transactions_count) \
SELECT :localeCode, transactions_count FROM pairwise_associations_transactions_count WHERE locale_id = :sourceLocaleCode;`;
