...
::: tip
Combination of short-lived `access token` and long-lived `refresh token` is focused on security and usability within SPA applications. For integration with other applications, where machine-to-machine communication is preferred, personal access tokens can be used.
:::
